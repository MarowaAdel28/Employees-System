 ...
 * employee application version 1

 * using c programming [array,struct,functions,conditions]

 * features new disply exit

 * new:

 *     can add till 100 employee
 ...
 *     when there is no employee yet will disply no employee meg
...
int currentDisplyEmpNo=0,reqEmpNo=0,empArrSize=0;
...
void drawHomePage(void){
	...
	gotoxy(40,8); // go to the middle of first rect (new button)
...
void runHomePage(void)
{
   ...
   //when press enter mean another page will disply so break from home page
...
void executeEnterKey(int positionY)
{
	...
	{
		if(empArrSize==100) drawFullArrMeg(); //no space to add new employee
...

	{
		...
		{
			//to print always last displayed employee

			reqEmpNo=currentDisplyEmpNo;
...
void displyNextPrevEmp(int positionX)
{
	if(positionX == 20) //press previou button
	{
		...
		if (reqEmpNo <0) reqEmpNo =empArrSize-1;  //get last employee

		displayEmp(reqEmpNo);

	}

	else if (positionX == 40) //press home button
	...
	else if (positionX == 60) //press next button
	{
		...
		if(reqEmpNo >= empArrSize) reqEmpNo=0; //get first employee
...
void drawFullArrMeg(void)
{
	...
	drawText(5,8,70,"Array is full!","RED");

	drawText(5,10,70,"please press any key to back to home","GREEN");
...
void drawNoEmpMeg()
{
	...
	drawText(25,13,30,"press enter key to back to home","GREEN");
